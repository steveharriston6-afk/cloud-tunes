...
#                 # WAV size is at bytes 4-8
#                 riff_size = struct.unpack('<I', data[4:8])[0]
#                 file_size = riff_size + 8
#                 # Calculate duration for standard PCM WAV
...
#         # Check for album name: (From _Album_) or (From “Album”) or (From "Album")
...
#     # Clean up title if it contains double spaces or trailing dashes
...
# def clean_database(songs_json_path):
...
#         # Override metadata for the three original tracks to keep them identical to original
#         if resolved_id in ORIGINAL_METADATA:
#             meta = ORIGINAL_METADATA[resolved_id]
#             song["title"] = meta["title"]
#             song["artist"] = meta["artist"]
#             song["album"] = meta["album"]
#             # Only use original local cover if not overriding in import
...
# def save_database(songs_json_path, songs):
#     # Run a final deduplication to clean up duplicates
#     seen_ids = set()
#     seen_combos = set()
#     final_songs = []
...
#         print(f"[+] Successfully wrote {len(final_songs)} unique tracks to songs.json")
#     except Exception as e:
#         print(f"[!] Error writing songs database: {e}")
...
#             covers_dir = os.path.join(os.path.dirname(__file__), "..", "public", "covers")
...
#     return title, artist, album, genre, duration, details, cover_url

# def import_gdrive_folder(folder_url):
...
#     except Exception as e:
#         print(f"[!] Error downloading page content: {e}")
#         return
...
#         filename = m.group(1)
#         pos = m.start()
#         window = html[max(0, pos-200):min(len(html), pos+200)]
...
#         print("[!] No audio files found in the folder. Check folder permissions.")
#         return
        
#     print(f"[+] Discovered {len(extracted_items)} audio tracks in Google Drive.")
    
#     songs_json_path = os.path.join(os.path.dirname(__file__), "..", "public", "songs.json")
#     songs_json_path = os.path.abspath(songs_json_path)
    
#     existing_songs = clean_database(songs_json_path)
...
#             # We already have this song in the database! Keep existing metadata/cover, only update URL
...
#         # New song! Download temporarily, extract metadata and cover, then clean up
#         try:
#             title, artist, album, genre, duration, details, cover_url = extract_gdrive_metadata_and_cover(file_id, filename, format_name)
#             song_entry = {
#                 "id": resolved_id,
#                 "title": title,
#                 "artist": artist,
...
#         except Exception as e:
#             skipped_files.append((filename, f"Failed to download/parse metadata: {e}"))
        
#     save_database(songs_json_path, existing_songs)
...
#     dest_dir = os.path.join(os.path.dirname(__file__), "..", "public", "tracks")
...
#             skipped_files.append((d, f"Directory does not exist ({source_label})"))
#             return
#         for filename in os.listdir(d):
#             full_path = os.path.join(d, filename)
#             if os.path.isdir(full_path):
#                 # Avoid recursing into system directories
#                 if filename in ('node_modules', '.git', '.gemini', 'venv', 'public', 'scripts', 'src', 'covers', 'tracks'):
#                     continue
#                 continue
#             if not filename.lower().endswith(extensions):
#                 skipped_files.append((filename, f"Unsupported file extension ({source_label})"))
#                 continue
...
#     # Synchronize deletions by filtering database selectively (preserve GDrive!)
#     existing_songs = [
#         s for s in existing_songs 
...
#         # Copy to public tracks first so we can parse from copied track
#         dest_path = os.path.join(dest_dir, filename)
#         if not os.path.exists(dest_path):
#             try:
#                 shutil.copy2(full_path, dest_path)
#             except Exception as e:
#                 skipped_files.append((filename, f"Failed to copy file to public/tracks: {e}"))
#                 continue
                
#         # Determine ID
#         file_hash = hashlib.md5(filename.encode('utf-8')).hexdigest()
        
#         # Extract embedded metadata if mutagen is available
#         if mutagen is not None:
#             try:
...
#         # Extract cover art (embedded or hashed fallback)
#         covers_dir = os.path.join(os.path.dirname(__file__), "..", "public", "covers")
#         os.makedirs(covers_dir, exist_ok=True)
...
#     env = load_env()
#     use_gdrive = env.get("VITE_USE_GOOGLE_DRIVE", "true").lower() == "true"
#     local_dir = env.get("LOCAL_SONGS_DIR", "")
    
#     if not use_gdrive and local_dir:
#         import_local_folder(local_dir)
#     else:
#         url_arg = env.get("GOOGLE_DRIVE_FOLDER_URL")
#         if len(sys.argv) > 1:
#             url_arg = sys.argv[1]
#         import_gdrive_folder(url_arg)


#!/usr/bin/env python3
"""
Fixed Google Drive Music Importer
- Now supports LARGE folders (400+ files) reliably via Google Drive API v3 (with optional API key)
- Falls back to improved HTML scraping for small folders or when no API key is set
- All other functionality (metadata extraction, covers, dedup, songs.json) unchanged
"""
...
if __name__ == '__main__':
    ...
    if not use_gdrive and local_dir:
        ...
        if len(sys.argv) > 1:
            url_arg = sys.argv[1]
        import_gdrive_folder(url_arg, env)
